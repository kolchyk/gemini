...
def render_chat_sidebar():
    """Renders the sidebar for gemini chat."""
    with st.sidebar:
        ...
        thinking_level = st.selectbox(
            "Мислення:",
            options=["low", "high"],
            index=0 if st.session_state.get('chat_thinking_level', 'high') == 'low' else 1,
        ...
        temperature = st.slider(
            ...
            min_value=0.0,
            max_value=2.0,
            value=st.session_state.get('chat_temperature', 0.95),
            step=0.1,
        ...
        _chat_uploader_key = f"chat_file_uploader_{st.session_state.get('chat_file_uploader_key', 0)}"
        st.file_uploader(
        ...
        st.divider()
        msg_count = len(st.session_state.get('gemini_chat_history', []))
        ...
        *Використовуйте 'Thinking' для складних міркувань.*
...
def render_gemini_chat():
    ...
    if prompt := st.chat_input("Введіть ваше повідомлення..."):
        # Get pending uploads from session state using current key
